...
func TestMatching(t *testing.T) {
	...
	t.Run("User Matching Routes (Protected)", func(t *testing.T) {
		...
		// ❗️ Добавлен 'tx'
		res, bodyStr = ts.SendRequest(t, tx, http.MethodGet, endpointWithParams, modelToken, nil)
		assert.Equal(t, http.StatusOK, res.StatusCode, "Model should find matches. Body: "+bodyStr)

		// 3. Ошибка: 401 (Без токена)
		// ❗️ Добавлен 'tx'
		res, _ = ts.SendRequest(t, tx, http.MethodGet, endpoint, "", nil)
		...
		// ❗️ Добавлен 'tx'
		res, bodyStr = ts.SendRequest(t, tx, http.MethodPost, "/api/v1/matching/models/search", empToken, searchBody)
		...
		// ❗️ Добавлен 'tx'
		res, bodyStr = ts.SendRequest(t, tx, http.MethodGet, compEndpoint, modelToken, nil)
		...
		// ❗️ Добавлен 'tx'
		res, bodyStr = ts.SendRequest(t, tx, http.MethodGet, "/api/v1/matching/compatibility?casting_id="+casting1.ID, modelToken, nil)
		...
		// ❗️ Добавлен 'tx'
		res, bodyStr = ts.SendRequest(t, tx, http.MethodGet, similarEndpoint, empToken, nil)
		...
		// ❗️ Добавлен 'tx'
		res, bodyStr = ts.SendRequest(t, tx, http.MethodGet, "/api/v1/matching/weights", modelToken, nil)
		...
		// ❗️ Добавлен 'tx'
		res, _ = ts.SendRequest(t, tx, http.MethodGet, "/api/v1/matching/castings/"+casting1.ID+"/stats", empToken, nil)
		assert.Equal(t, http.StatusOK, res.StatusCode, "Casting stats should be OK")

		// ❗️ Добавлен 'tx'
		res, _ = ts.SendRequest(t, tx, http.MethodGet, "/api/v1/matching/models/"+modelProfile.ID+"/stats", modelToken, nil)
	...
	t.Run("Admin Matching Routes (Admin Only)", func(t *testing.T) {
		...
		// 1. Ошибка 403 (Модель)
		// ❗️ Добавлен 'tx'
		res, bodyStr := ts.SendRequest(t, tx, http.MethodPut, weightsEndpoint, modelToken, weightsBody)
		assert.Equal(t, http.StatusForbidden, res.StatusCode, "Model should be forbidden. Body: "+bodyStr)

		// 2. Ошибка 403 (Работодатель)
		// ❗️ Добавлен 'tx'
		res, _ = ts.SendRequest(t, tx, http.MethodPut, weightsEndpoint, empToken, weightsBody)
		assert.Equal(t, http.StatusForbidden, res.StatusCode, "Employer should be forbidden")

		// 3. Успешно (Админ)
		// ❗️ Добавлен 'tx'
		res, bodyStr = ts.SendRequest(t, tx, http.MethodPut, weightsEndpoint, adminToken, weightsBody)
		...
		// ❗️ Добавлен 'tx'
		res, _ = ts.SendRequest(t, tx, http.MethodGet, statsEndpoint, modelToken, nil)
		assert.Equal(t, http.StatusForbidden, res.StatusCode, "Stats is admin-only")
		// ❗️ Добавлен 'tx'
		res, bodyStr = ts.SendRequest(t, tx, http.MethodGet, statsEndpoint, adminToken, nil)
		...
		// ❗️ Добавлен 'tx'
		res, _ = ts.SendRequest(t, tx, http.MethodPost, recalcEndpoint, empToken, nil)
		assert.Equal(t, http.StatusForbidden, res.StatusCode, "Recalculate is admin-only")
		// ❗️ Добавлен 'tx'
		res, bodyStr = ts.SendRequest(t, tx, http.MethodPost, recalcEndpoint, adminToken, nil)
		...
		// ❗️ Добавлен 'tx'
		res, _ = ts.SendRequest(t, tx, http.MethodGet, logsEndpoint, modelToken, nil)
		assert.Equal(t, http.StatusForbidden, res.StatusCode, "Logs are admin-only")
		// ❗️ Добавлен 'tx'
		res, bodyStr = ts.SendRequest(t, tx, http.MethodGet, logsEndpoint+"?page=1&pageSize=5", adminToken, nil)
		...
		// 1. Ошибка 403 (Модель)
		// ❗️ Добавлен 'tx'
		res, _ = ts.SendRequest(t, tx, http.MethodPost, batchEndpoint, modelToken, batchBody)
		assert.Equal(t, http.StatusForbidden, res.StatusCode, "Batch match is admin-only")

		// 2. Ошибка 400 (Админ, неверное тело)
		// ❗️ Добавлен 'tx'
		res, bodyStr = ts.SendRequest(t, tx, http.MethodPost, batchEndpoint, adminToken, gin.H{})
		assert.Equal(t, http.StatusBadRequest, res.StatusCode, "Should fail validation. Body: "+bodyStr)

		// 3. Успешно (Админ)
		// ❗️ Добавлен 'tx'
		res, bodyStr = ts.SendRequest(t, tx, http.MethodPost, batchEndpoint, adminToken, batchBody)
...
func TestMatching_AdminSecurity(t *testing.T) {
	...
	for _, endpoint := range adminEndpoints {
		...
		if endpoint == "/api/v1/admin/matching/weights" {
			...
			// ❗️ Добавлен 'tx'
			res, bodyStr := ts.SendRequest(t, tx, http.MethodPut, endpoint, adminToken, weightsBody)
			...
			// ❗️ Добавлен 'tx'
			res, bodyStr := ts.SendRequest(t, tx, http.MethodPost, endpoint, adminToken, batchBody)
